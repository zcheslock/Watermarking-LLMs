...
`zscore_vs_T_varying_delta.png` and `zscore_vs_T_varying_gamma.png` visualize how detector confidence changes with sequence length `T` under different watermark settings. `tpr_fpr_table.png` summarizes the detection tradeoff more directly by comparing positive-rate behavior under thresholding. A user running this repo should expect to see measurable separation between watermarked and unwatermarked behavior, even though the exact numerical values may differ from the original paper because of model scale and experimental simplifications.

![TPR / FPR summary](results/tpr_fpr_table.png)
